(() => {
  ...
  window.ENV.COLUMNS = window.ENV.COLUMNS || "120";
  window.ENV.LINES = window.ENV.LINES || "40";

  Module.locateFile = (path) => "/fairlanes/" + path;
  Module.mainScriptUrlOrBlob = "/fairlanes/fairlanes.js";

  let runtimeReady = false;
  let pendingResize = {
    cols: Number(window.ENV.COLUMNS),
    rows: Number(window.ENV.LINES),
  };

  const terminalEl = document.getElementById("terminal");
  if (!terminalEl) throw new Error("Missing #terminal element");
  ...
  function rememberSize() {
    window.ENV.COLUMNS = String(term.cols);
    window.ENV.LINES = String(term.rows);
  ...
  function updateTtySize() {
    ...
    if (typeof mod._ftxui_on_resize === "function") {
      mod._ftxui_on_resize(term.cols, term.rows);
      return true;
    }
  ...
  function fitTerminal() {
    ...
    try {
      ...
      console.log(
        ...
        "x",
        term.rows,
        updated ? "(tty updated)" : runtimeReady ? "(no tty hook)" : "(runtime not ready)"
      );
    } catch (err) {
  ...
  term.onData(() => {
    // Browser keydown bridge is the sole Fairlanes input path.
    // Do not echo or enqueue here, or web input doubles/drifts.
  });
